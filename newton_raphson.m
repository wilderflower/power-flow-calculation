clear;clc
%{潮流计算总共有六个变量
电压，相角，发出的无功，有功，消耗的无功，有功。
其中消耗的无功，有功为不可控变量；电压、相角为干扰变量，电压不能太大，所以他是一个约束变量；发出的有功，无功为可控的自变量，一台发电机发出的有功，无功为一个定值，不能太大。
所以约束条件共有三个，电压，发出的有功，发出的无功。它们分别大于最小值，小于最大值。
}%
...
Q=imag(S);
%赋初值
U=ones(1,n);U(n)=1.06;%节点电压幅值 设平衡节点电压保持1.06
e=zeros(1,n); %节点电压角度
...
while max(fx)>1e-5
    ...
    for i=1:n-1
        for j=1:n %写成修正方程的形式,求deltaP deltaQ
...
%求节点注入的净功率
i=n;
for j=1:n %计算平衡节点的发电机发出的有功和无功功率
